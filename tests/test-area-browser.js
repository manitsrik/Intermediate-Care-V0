...
(async () => {
  try {
    ...
    await check('Dashboard keeps region data without rendering a duplicate table', 'dashCache.total === 6 && document.querySelectorAll("main .area-table").length === 0 && !!document.querySelector(".area-summary-button") && dashCache.districts.reduce((s,r) => s+r.total,0) === 6');
    await screenshot('dashboard-areas-desktop.png');
    await evaluate('openAreaSummary()');
    await check('Area summary opens as one compact dialog', '!document.getElementById("areamodal").hidden && document.querySelectorAll("#areamodal .area-table").length === 1');
    await check('Zero-count regions are hidden by default', 'document.querySelectorAll(".area-table tbody tr").length === dashCache.districts.filter(r => r.total > 0).length');
    await screenshot('dashboard-area-tables-desktop.png');
    await evaluate('setAreaTab("tambon")');
    await check('Area tabs replace the table in place', 'document.querySelectorAll(".area-table").length === 1 && document.querySelector("[role=tab][aria-selected=true]").textContent.includes("ตำบล")');
    await evaluate('toggleAreaEmpty()');
    await check('All tambons can be revealed without adding another card', 'document.querySelectorAll(".area-table tbody tr").length === 6 && !!document.querySelector(".area-more")');
    await evaluate('toggleAreaRows()');
    await check('Expanded area list shows every tambon', 'document.querySelectorAll(".area-table tbody tr").length === dashCache.tambons.length');
    await evaluate('areaShowEmpty = false; areaRowsExpanded = false; setAreaTab("district"); closeAreaSummary()');
    ...
    await evaluate('openAreaPatients("tambon", 0)');
    await check('Tambon drilldown matches total', 'sheetRows.length === 2 && !document.getElementById("listmodal").hidden');
    await evaluate('closeList(); openAreaPatients("tambon", 0, "active")');
    await check('Status count drills into the correct subset', 'sheetRows.length === dashCache.tambons[0].active && sheetRows.every(p => p.status !== "closed")');
    ...
    await evaluate('openAreaSummary()');
    await check('Mobile area summary fits inside the viewport', 'document.querySelector(".area-summary-sheet").getBoundingClientRect().width <= innerWidth - 30');
    await screenshot('dashboard-area-table-mobile.png');
    await evaluate('closeAreaSummary(); go("patient", "TEST001")');
